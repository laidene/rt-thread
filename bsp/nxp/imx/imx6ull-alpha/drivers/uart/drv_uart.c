...
static struct imx_uart _s_uart[eDevUart_Max] = {
#ifdef BSP_USING_UART1
{
    .name           = "uart0",
...
static void _uart_gpio_init(struct imx_uart *device)
{
    ...
    {
        io_mux_pad_init(&device->gpio[i]);
    }
}


static rt_err_t _uart_ops_configure( struct rt_serial_device *dev, struct serial_configure *cfg )
{
    ...
    periph->UCR1 &= ~UART_UCR1_UARTEN_MASK; /* UARTx_UCR1 UARTEN=0 */

    periph->UFCR &= ~UART_UFCR_RFDIV_MASK;  /* UARTx_UFCR RFDIV=0 */
    periph->UFCR |=  UART_UFCR_RFDIV(5);    /* RFDIV=1 */

    RT_ASSERT(cfg->baud_rate <= BAUD_RATE_921600);

    /* BaudRate = Ref Freq / (16 * (UBMR+1) / (UBIR + 1)  ) */
    /* BaudRate = 80M / (16 * (694+1) / (15 + 1)  ) */
    periph->UBIR = UART_UBIR_INC(15);                                       /* UARTx_UBIR INC=15 */
    periph->UBMR = UART_UBMR_MOD(HW_UART_BUS_CLOCK / cfg->baud_rate - 1);   /* UART7_UBMR MOD= 80M / 115200 - 1 = 694 */
    ...
    {
        case DATA_BITS_7:
            reg_value |= UART_UCR2_WS(0);     /* UARTx_UCR2 WS=0 7-bit*/
            break;
        default:
            reg_value |= UART_UCR2_WS(1);     /* UARTx_UCR2 WS=1 8-bit*/
            break;
    }
    ...
    {
        case STOP_BITS_2:
            reg_value |= UART_UCR2_STPB(1);     /* UARTx_UCR2 STPB=1 2-bit*/
            break;
        default:
            reg_value |= UART_UCR2_STPB(0);     /* UARTx_UCR2 STPB=0 1-bit*/
            break;
    }
    ...
    {
        case PARITY_ODD:
            reg_value |= UART_UCR2_PREN(1);     /* UARTx_UCR2 PREN=1 Enable parity generator and checker */
            reg_value |= UART_UCR2_PROE(1);     /* UARTx_UCR2 PROE=1 Odd parity */
            break;
        case PARITY_EVEN:
            reg_value |= UART_UCR2_PREN(1);     /* UARTx_UCR2 PREN=1 Enable parity generator and checker */
            reg_value |= UART_UCR2_PROE(0);     /* UARTx_UCR2 PROE=0 Even parity */
            break;
        default:
            reg_value |= UART_UCR2_PREN(0);     /* UARTx_UCR2 PREN=0 Disable parity generator and checker */
            reg_value |= UART_UCR2_PROE(0);     /* UARTx_UCR2 PROE=0 No parity */
            break;
    }

    periph->UCR3 |= UART_UCR3_RXDMUXSEL(1);     /* UART Control Register 3 RXDMUXSEL=1 so that this bit should always be set */
    periph->UCR2 |= reg_value | UART_UCR2_IRTS(1) | UART_UCR2_TXEN(1) | UART_UCR2_RXEN(1);  /* IRTS=1 TXEN=1 RXEN=1 */
    periph->UCR1 |= UART_UCR1_UARTEN(1);                                                    /* UARTx_UCR1 UARTEN=1 */

    return RT_EOK;
}

static rt_err_t _uart_ops_control( struct rt_serial_device *dev, int cmd, void *arg )
{
    struct imx_uart *uart = RT_NULL;
    UART_Type *periph = RT_NULL;
    rt_err_t result;
    ...
    switch (cmd)
    {
        case RT_DEVICE_CTRL_CLR_INT:
            ...
            periph->UCR4 |=  UART_UCR4_DREN(0);   /* DREN=0 Disable Receive Data Ready interrupt */
            break;

        case RT_DEVICE_CTRL_SET_INT:
            periph->UCR4 |=  UART_UCR4_DREN(1);   /* DREN=1 Enable Receive Data Ready interrupt */
...
static int _uart_ops_putc( struct rt_serial_device *dev, char ch )
{
    struct imx_uart *uart = RT_NULL;
    UART_Type *periph = RT_NULL;
    ...
    while (0 == (periph->USR2 & UART_USR2_TXDC_MASK));  /* TXDC=0 Transmitter Complete. */
    periph->UTXD = ch;                                  /* TX_DATA=ch */
...
static int _uart_ops_getc( struct rt_serial_device *dev )
{
    ...
    ch = (0 == (periph->USR2 & UART_USR2_RDR_MASK)) ? -1 : periph->URXD;    /* Receive data ready ? -1 : RX_DATA */
    return ch;
}


static const struct rt_uart_ops _k_uart_ops = {
    _uart_ops_configure,
    _uart_ops_control,
    _uart_ops_putc,
    _uart_ops_getc,
...
int rt_hw_uart_init(void)
{
    ...
    {
        _s_uart[idx].periph.vaddr = _s_uart[idx].periph.paddr;
